...
object HeaderUtils {
    ...
    fun toHeaders(feignHeaders: Map<String, Collection<String>>): HttpHeaders {
        ...
        feignHeaders.keys.forEach { key ->
            headers.put(key, getHeaderValues(feignHeaders, key))
        }
        return headers
    }
    ...
    fun toHeadersWithEnumHeaders(
        ...
        while (headerNames.hasMoreElements()) {
            ...
            if (previous == null) {
                headers.put(name, Collections.list(getHeaders(name)))
            } else {
    ...
    fun toHeaders(
        ...
        for (name in headerNames) {
            val previous = headers[name]
            if (previous == null) {
                headers.put(name, ArrayList(getHeaders(name)))
            } else {
                previous.add(getHeader(name))
                headers.put(name, previous)
            }
    ...
    private fun append(headers: HttpHeaders, entry: Map.Entry<String, String>) {
        ...
        if (previous == null) {
            headers.put(entry.key, Collections.singletonList(entry.value))
        } else {
            previous.add(entry.value)
            headers.put(entry.key, previous)
        }
    }
