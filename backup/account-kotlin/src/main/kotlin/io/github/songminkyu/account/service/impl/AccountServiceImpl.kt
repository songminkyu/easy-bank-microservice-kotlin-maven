...
class AccountServiceImpl(
    ...
    override fun fetchAccount(mobileNumber: String): CustomerDTO {
        val customer = customerRepository.findByMobileNumber(mobileNumber)
            .orElseThrow { EntityNotFoundException(Customer::class.java, "mobileNumber", mobileNumber) }

        val account = accountRepository.findByCustomerId(customer.customerId!!)
            .orElseThrow { EntityNotFoundException(Account::class.java, "customerId", customer.customerId.toString()) }
    ...
    override fun geCreatorUsername(accountNumber: Long): String {
        val revision = accountRepository.findRevision(accountNumber, 1)
            .orElseThrow { EntityNotFoundException(Revision::class.java, "accountNumber", accountNumber.toString()) }

        val metadata = revision.metadata
        // RevisionEntity 타입을 명시적으로 지정
        return when (val revisionEntity = metadata.getDelegate<RevisionEntity>()) {
            is RevisionEntity -> revisionEntity.username ?: ""
            else -> ""
        }
    }
    ...
    override fun updateAccount(accountNumber: Long, customer: CustomerDTO): Boolean {
        ...
        val account = accountRepository.findById(accountNumber)
            .orElseThrow { EntityNotFoundException(Account::class.java, "AccountNumber", accountNumber.toString()) }

        accountMapper.partialUpdate(account, accountDTO)
        val updatedAccount = account.copy(accountNumber = accountNumber)
        ...
        val customerEntity = customerRepository.findById(customerId)
            .orElseThrow { EntityNotFoundException(Customer::class.java, "CustomerID", customerId.toString()) }
    ...
    override fun updateCommunicationStatus(accountNumber: Long): Boolean {
        ...
        val updatedAccount = account.copy(communicationSw = true)
